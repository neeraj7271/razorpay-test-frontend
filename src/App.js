...
function App() {
  ...
  const [loading, setLoading] = useState(true);
  const [selectedAddons, setSelectedAddons] = useState({}); // State to track selected addons
  ...
  const handleAddonChange = (addon) => {
    setSelectedAddons((prev) => ({
      ...prev,
      [addon]: !prev[addon], // Toggle the selected state
    }));
  };
  ...
  const handleSubscriptionPayment = async (plan) => {
    ...
    try {
      const response = await axios.post('https://razorpay-testing-backend.vercel.app/api/create-subscription', {
        ...
        customerDetails: customerDetails,
        totalCount: 12,
        customerId: "cust_QC7ETbBxDPSfZq"
      });

      if (response.data.success) {
        const subscription = response.data.subscription;

        // Prepare addons array based on selected addons
        const addons = Object.keys(selectedAddons)
          .filter((addon) => selectedAddons[addon]) // Get only selected addons
          .map((addon) => {
            switch (addon) {
              case 'extraStorage':
                return { amount: 500, description: "Extra Storage", quantity: 1 };
              case 'prioritySupport':
                return { amount: 300, description: "Priority Support", quantity: 1 };
              default:
                return null;
            }
          })
        ...
        await addAddons(subscription.id, addons);
  ...
  const addAddons = async (subscriptionId, addons) => {
    try {
      const response = await axios.post('https://razorpay-testing-backend.vercel.app/api/add-addon', {
        subscriptionId: subscriptionId,
        addons: addons // Send the array of addons
      });

      if (response.data.success) {
        console.log("Addons added successfully:", response.data.addons);
      } else {
        console.error("Failed to add addons:", response.data.error);
      }
    } catch (error) {
            ...
            <div className="card-content">
              ...
              <div className="addons">
                ...
                <label>
                  <input
                    type="checkbox"
                    checked={selectedAddons.extraStorage || false}
                    onChange={() => handleAddonChange('extraStorage')}
                  />
                  Extra Storage (₹500)
                </label>
                <label>
                  <input
                    type="checkbox"
                    checked={selectedAddons.prioritySupport || false}
                    onChange={() => handleAddonChange('prioritySupport')}
                  />
